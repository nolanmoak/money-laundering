...
const Card = React.forwardRef<ViewRef, ViewProps>(({ className, ...props }, ref) => (
  <View
    ref={ref}
    className={cn('border-border bg-card shadow-foreground/10 rounded-lg border shadow-sm', className)}
    {...props}
  />
));
...
const CardTitle = React.forwardRef<TextRef, TextProps>(({ className, ...props }, ref) => (
  <Text
    ...
    aria-level={3}
    ref={ref}
    className={cn('text-card-foreground text-2xl font-semibold leading-none tracking-tight', className)}
    {...props}
  />
));
